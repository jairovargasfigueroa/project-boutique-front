...
export interface CartStore {
  items: CartItem[]
  agregarProducto: (producto: Producto, variante: ProductoVariante, cantidad?: number) => void
    ...
    (set, get) => ({
      ...
      agregarProducto: (producto, variante, cantidad = 1) => {
        console.log('🛒 AGREGANDO AL CARRITO:', {
          ...
          variante: `${variante.color} - ${variante.talla}`,
          cantidad,
        ...
        if (existingItem) {
          console.log('✅ Producto ya existe, aumentando cantidad de', existingItem.cantidad, 'a', existingItem.cantidad + cantidad)
          set({
          ...
          const newItem: CartItem = {
            ...
            variante,
            cantidad,
            precioUnitario: variante.precio_venta || 0  // snapshot del precio
          }
        ...
        console.log('📦 ESTADO ACTUAL DEL CARRITO:', {
          ...
          total: updatedItems.reduce((total, item) => total + item.precioUnitario * item.cantidad, 0)
        })
      },
      ...
      quitarProducto: (id) => {
        ...
        if (itemToRemove) {
          console.log('🗑️ QUITANDO DEL CARRITO:', {
            ...
            color: itemToRemove.variante.color
          })
        }
