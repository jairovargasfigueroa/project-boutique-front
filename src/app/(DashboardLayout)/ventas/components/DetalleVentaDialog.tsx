...
const DetalleVentaDialog = ({
  ...
  } = useVentaDetalle(ventaInicial?.id);
  const {
    cuotas,
    loading: loadingCuotas,
    refetch: refetchCuotas,
  } = useCuotas(ventaInicial?.id);
  const {
    pagos: pagosHook,
    loading: loadingPagos,
    refetch: refetchPagos,
  } = usePagos(ventaInicial?.id);
  ...
  // Usar pagos del detalle de venta si están disponibles, sino del hook
  const pagos = venta?.pagos || pagosHook;

  useEffect(() => {
    if (open && ventaInicial) {
      refetchVenta();
      if (ventaInicial.tipo_venta === "credito") {
        refetchCuotas();
      }
      refetchPagos();
    }
      ...
      <DialogTitle>
        <Box display="flex" justifyContent="space-between" alignItems="center">
          ...
          <Chip
            ...
            color={venta.tipo_venta === "contado" ? "primary" : "secondary"}
          />
        </Box>
      </DialogTitle>
            ...
            <Box>
              ...
              <Box display="flex" justifyContent="space-between" mb={1}>
                ...
                <Chip
                  label={venta.estado}
                  color={
                    venta.estado === "pagado" || venta.estado === "completado"
                      ? "success"
              ...
              <Box display="flex" justifyContent="space-between" mb={0.5}>
                <Typography variant="body2" color="text.secondary">
                  {venta.tipo_venta === "credito" ? "Subtotal:" : "Total:"}
                </Typography>
                <Typography variant="body2" fontWeight={500}>
                ...
                <>
                  <Box display="flex" justifyContent="space-between" mb={0.5}>
                    ...
                    >
                      + Bs{" "}
                      {(
                        parseFloat(venta.total_con_interes || venta.total) -
                  ...
                  <Box display="flex" justifyContent="space-between" mb={0.5}>
                    ...
                    >
                      Bs {parseFloat(venta.cuota_mensual || "0").toFixed(2)}
                    </Typography>
                  </Box>
                </>
              )}
                    ...
                    venta.detalles.map((detalle) => (
                      ...
                      >
                        ...
                        <Box textAlign="right">
                          ...
                          <Typography variant="body2" fontWeight={500}>
                            Bs {parseFloat(detalle.sub_total).toFixed(2)}
                          </Typography>
                        </Box>
                      </Box>
                    ))
                    ...
                    <Box maxHeight="300px" overflow="auto">
                      {pagos.map((pago) => (
                        ...
                        >
                          <Box flex={1}>
                            <Typography variant="body2" fontWeight={500}>
                              {pagoService.formatearMetodoPago(
                                pago.metodo_pago
                              )}
                              {pago.cuota && ` - Cuota ID ${pago.cuota}`}
                            </Typography>
                            <Typography
                              variant="caption"
